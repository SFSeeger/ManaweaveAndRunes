...
import static io.github.sfseeger.manaweave_and_runes.core.init.MRMenuInit.RUNEWROUGHT_BENCH_MENU_TYPE;

public class RunewroughtBenchMenu extends AbstractContainerMenu {
    ...
    private final RunewroughtBenchBlockEntity blockEntity;
    private final ItemStackHandler itemHandler;
    private final SpellCircleSlot[] spellCircleSlots = new SpellCircleSlot[12];
    private IItemHandler upgradeableItemHandler;

    public RunewroughtBenchMenu(int containerId, Inventory playerInventory, FriendlyByteBuf buffer) {
        this(containerId,
                playerInventory,
             (RunewroughtBenchBlockEntity) playerInventory.player.level().getBlockEntity(buffer.readBlockPos()),
                ContainerLevelAccess.NULL);
    }

    public RunewroughtBenchMenu(int containerId, Inventory playerInventory, RunewroughtBenchBlockEntity blockEntity,
            ContainerLevelAccess access) {
        super(RUNEWROUGHT_BENCH_MENU_TYPE.get(), containerId);
        this.access = access;
        ...
        this.addSlot(new SlotItemHandler(itemHandler, 0, 16, 33) {
            ...
            public void onTake(Player player, ItemStack stack) {
                if (stack.getItem() instanceof IUpgradable) {
                    RunewroughtBenchMenu.this.upgradeableItemHandler = null;
                    for (int i = 0; i < 12; i++) {
            ...
            public void set(ItemStack stack) {
                if (stack.getItem() instanceof IUpgradable) {
                    RunewroughtBenchMenu.this.upgradeableItemHandler =
                            stack.getCapability(Capabilities.ItemHandler.ITEM);
                    for (int i = 0; i < 12; i++) {
                        spellCircleSlots[i].setItemHandler(RunewroughtBenchMenu.this.upgradeableItemHandler);
                    }
                }
                super.set(stack);
            }
    ...
    public boolean stillValid(Player player) {
        return AbstractContainerMenu.stillValid(this.access,
                                                player, MRBlockInit.RUNEWROUGHT_BENCH_BLOCK.get());
    }
